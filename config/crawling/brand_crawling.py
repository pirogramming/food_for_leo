...
def brand_crawling():
    ...
    brand_list_0 = [i.getText() for i in soup.select('.category-sub-new tr:nth-of-type(1) ul a')]
    global brand_list
    brand_list += [brand_list_0]
    ...
    brand_list_1 = [i.text.split(' ')[0] for i in soup.select('.cate a')]
    ...
    html = req.text
    soup = bs(html, 'html.parser')
    brand_list_3 = [i.text.strip().split('(')[0] for i in soup.select('.nano-content.nano-content-scroll label')]
